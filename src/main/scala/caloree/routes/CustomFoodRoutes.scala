...
object CustomFoodRoutes {
  def routes[F[_]: Monad](
      implicit
      go: Execute[F, (EntityId[CustomFood], EntityId[User]), Option[CustomFood]],
      gm: Execute[F, (Description, EntityId[User], Page, Int), List[CustomFoodPreview]]
  ): AuthedRoutes[User, F] = {
    val dsl = Http4sDsl[F]
    import dsl._

    AuthedRoutes.of {
      case GET -> _ :? CustomFoodIdP(id) as User(uid, _, _) => go.execute((id, uid)).asResponse
